...
const ResultCardChart = ({ score, data }: {
  ...
  const render = React.useCallback(() => {
    ...
    for (const [_score, _count] of entries) {
      const scoreNum = Number(_score);
      ctx.lineTo(
        (scoreNum - scoreMin) / (scoreMax - scoreMin) * w + p,
        (1 - (_count - countMin) / (countMax - countMin)) * h + up
      );
    ...
    <AppContent className='MyRatingResult'>
      <div className='MyRatingResult_Cards'>
        ...
        <ResultCard
          ...
          emoji='📐'
          header='Твій бал ЗНО з математики краще ніж у 76% абітурієнтів'
          text='які здавали цей предмет в 2021 році'
        ...
        <ResultCard
          ...
          emoji='🧬'
          header='Твій бал ЗНО з географї краще ніж у 76.4% абітурієнтів'
          text='які здавали цей предмет в 2021 році'
      ...
      <div className='MyRatingResult_Bottom'>
        <Button
          onClick={() => {}}
        >
          Поділитись
