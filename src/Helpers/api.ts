...
export const baseURL = DEV
  ...
  : "https://studsearch.org";
export const endpoint = DEV
  ? "https://server.studsearch.org:2324"
  : "https://server.studsearch.org:2323";
...
export class DataSource<T> extends UpdateEmitter<[]> {
  ...
  async load() {
    ...
    const myID = ++this.id;
    let result = await this.loadCallback(
      this.query,
      this.pageCount,
      this.items.length
    );
    if (myID !== this.id) return;

    if (result === undefined || result.length < this.pageCount)
      this.complete = true;
    if (result !== undefined) this.items.push(...result);
    this.loading = false;
    this.update();
  }
...
export const takeString = (entry?: string | FieldEntry, lang?: string) => {
  ...
  if (lang && localesMap[lang]) lang = localesMap[lang];
...
export const getRegions = async (): Promise<RegionsData> => {
  ...
  if (Array.isArray(result)) return { regions: result, yours: null };
  return result;
};
...
export const getUniversities = (
  ...
  __reqjson("/universities/" + (regionId !== undefined ? regionId : ""), {
    count,
    offset,
    query,
  });
export const Universities = <T>(
  customizer: (university: University) => T,
  regionId?: number
) =>
  new DataSource<T>(async (query, count, offset) => {
    return (await getUniversities(query, regionId, count, offset)).map(
      customizer
    );
  });
...
export const getFaculties = (
  ...
  __reqjson("/faculties/" + (universityId !== undefined ? universityId : ""), {
    count,
    offset,
    query,
  });
export const Faculties = <T>(
  customizer: (faculty: Faculty) => T,
  universityId?: number
) =>
  new DataSource<T>(async (query, count, offset) => {
    return (await getFaculties(query, universityId, count, offset)).map(
      customizer
    );
  });
...
export const getSpecialities = (
  ...
  __reqjson(
    "/specialities/" + (universityId !== undefined ? universityId : ""),
    { count, offset, query }
  );
export const Specialities = <T>(
  customizer: (speciality: Speciality) => T,
  universityId?: number
) =>
  new DataSource<T>(async (query, count, offset) =>
    (await getSpecialities(query, universityId, count, offset)).map(customizer)
  );
...
export const count = async (): Promise<{
  studentsCount: number;
...
export function fetchCabinetData(
  setDataState: React.Dispatch<React.SetStateAction<CabinetData | undefined>>
) {
...
export interface PostCabinetData {
  name: string;
  about: string;
  gender: "male" | "female";
  universityId: number;
...
export function postCabinetData(
  serverData: CabinetData | undefined,
  newData: PostCabinetData | undefined,
  onUpdateState: React.Dispatch<React.SetStateAction<number>>
) {
  if (serverData) {
    const {
      name,
    ...
    fetch("https://server.studsearch.org:2324/v2/cabinet", {
      method: "POST",
      headers: {
...
export function fetchSocials(
  setDataState: React.Dispatch<React.SetStateAction<SocialsData | undefined>>
) {
  const response = fetch(
    "https://server.studsearch.org:2324/v2/cabinet/contacts",
    {
      credentials: "include",
    }
  );
  response
    .then((response) => response.json())
    .then((data: SocialsData) => {
      setDataState(data);
    });
}

export function postSocials(
  serverData: SocialsData,
  newData: SocialsData,
  onUpdateState: React.Dispatch<React.SetStateAction<number>>
) {
  newData = {...serverData, ...newData}
  fetch("https://server.studsearch.org:2324/v2/cabinet/contacts", {
    method: "POST",
    headers: {
      "Content-Type": "application/json;charset=utf-8",
    },
